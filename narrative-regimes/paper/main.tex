...
orthogonal enhancement toggles (Ledoit--Wolf covariance shrinkage, regime-specific risk
...
with a max-drawdown tie-break. Nine of the thirty changes were accepted (see
replication artefact \texttt{results/tables/iteration\_log.csv}). The final composite,
\emph{Narrative Regimes (iterated)}, enables LW shrinkage ($\alpha=0.2$), regime-specific
...
baseline it improves CRRA-CE by $+27$ bps to $3.04\%$ (SE $24$ bps), lifts Sharpe to
$0.32$, reduces maximum drawdown by $4$ pp to $-18.7\%$, and cuts monthly turnover
approximately in half (from $7.5\%$ to $3.7\%$). The gains are concentrated in the
...
same range, so the framework's lead over both benchmarks is preserved (79 bps over risk
